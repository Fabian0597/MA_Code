...
features_of_interest=( "D:I_ist/X" "D:I_soll/X" "D:P_mech./X" "C:x_top" "C:z_top" "C:y_nut" "C:z_nut" "D:x_nut" "D:y_top" "D:z_top" "D:z_bottom" "S:Actual_rotational_speed[µm/s]" "S:Actual_position_of_the_position_encoder(dy/dt)[µm/s]"
        "S:Actual_position_of_the_motor_encoder(dy/dt)[µm/s]" )
...
MMD_layer_activation_flag_FULL=( True True False True True True )
...
do
    for feature_of_interest in ${features_of_interest[@]}; do
        for GAMMA in ${GAMMAs[@]}; do
            ...
            python3 main.py $experiment_name $num_epochs $GAMMA $GAMMA_reduction $num_pool ${MMD_layer_activation_flag_CNN[@]} $feature_of_interest
            python3 main.py $experiment_name $num_epochs $GAMMA $GAMMA_reduction $num_pool ${MMD_layer_activation_flag_FC[@]} $feature_of_interest
            python3 main.py $experiment_name $num_epochs $GAMMA $GAMMA_reduction $num_pool ${MMD_layer_activation_flag_FULL[@]} $feature_of_interest
        done
    done
done